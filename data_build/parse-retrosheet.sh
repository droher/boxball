...
# Now we parse the event files with the Chadwick toolkit, adding newlines at end of file
daily_parse='cwdaily -q -y {} {}* >> /parsed/daily.csv && echo "" >> /parsed/daily.csv'
comment_parse='cwcomment -q -y {} {}* >> /parsed/comment.csv && echo "" >> /parsed/comment.csv'
game_parse='cwgame -q -y {} -f 0-83 -x 0-94 {}* >> /parsed/game.csv && echo "" >> /parsed/game.csv'
sub_parse='cwsub -q -y {} {}* >> /parsed/sub.csv && echo "" >> /parsed/sub.csv'
event_parse='cwevent -q -y {} -f 0-96 -x 0-62 {}* >> /parsed/event.csv && echo "" >> /parsed/event.csv'

event_folders=(asg post regular)
chadwick_output_types=(daily comment event game sub)
parse_funcs=("${daily_parse}" "${sub_parse}" "${comment_parse}"  "${game_parse}" "${event_parse}")
...
for func in "${parse_funcs[@]}"; do
    ...
    for folder in "${event_folders[@]}"; do
        ...
        cd /retrosheet/event/"${folder}" && \
            ...
            xargs -P 1 -i bash -c "echo {} && shopt -s nullglob && ${func}"
    done
done
