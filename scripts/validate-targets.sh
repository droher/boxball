...
run_target() {
  ...
  case "$target" in
    postgres|postgres-columnar)
      ...
      # Poll until baseballdatabank.teams is queryable.
      wait_for "teams loaded" 360 \
        docker exec "$name" psql -U postgres -tAc "SELECT 1 FROM baseballdatabank.teams LIMIT 1" || {
    ...
    mysql)
      ...
      wait_for "mysql up" 480 \
        docker exec "$name" mysql -uroot -e "SELECT 1" || { cleanup "$name"; return 1; }
      wait_for "teams loaded" 480 \
        docker exec "$name" mysql -uroot -N -e "SELECT 1 FROM baseballdatabank.teams LIMIT 1" || {
          cleanup "$name"; return 1; }
    ...
    sqlite)
      ...
      wait_for "sqlite-web ready" 120 \
        docker exec "$name" test -s /db/boxball.db || { cleanup "$name"; return 1; }
