...
Active: none. Next up: Wave 6 shipping infra (**PLE-357/358/359**) — `make bake-push` is one `docker login` away from being a full release pipeline. Wave 5 done (see entry below).

PLE-353 + PLE-354 + PLE-355 done — single composite squash-merge into local `next` from `feature/ple-353-355-data-refresh`. **Refreshes both upstream data sources to current vintage and bumps `VERSION` to `2026.0.0`.**
