...
PLE-356 cutover done — shipped Boxball 2026.0.0. Squash-merged `next` into `master` (commit `2fc6f26` for 356b carrying forward), then layered three release-infra commits directly on master: `d47a1ec` (build-push job → self-hosted Mac runner), `f5bc80b` (per-job DOCKER_CONFIG isolation; force-pushed over earlier verbose commit), `a797431` (file-backed `docker-credential-runnerstore` with self-test). Self-hosted Mac runner re-registered from `droher/create-claude-code-app` to `droher/boxball` (default labels `self-hosted, macOS, ARM64`). Master branch protection temporarily lifted `allow_force_pushes` for the squash-history rewrite, restored immediately after. Full release dispatched via `workflow_dispatch` (run `25215346045`): every job green — `resolve` → `build-push` (3 bake waves, all `--push` to Docker Hub) → `validate-runtime` (5 load targets pass against just-pushed `:2026.0.0` manifests) → `promote-latest` (`<stage>-latest` retag for all 9 stages) → `bundles` (parquet + csv tar.zst) → `release` (GH Release `v2026.0.0` with bundles attached, body extracted from `CHANGELOG.md`).

**Self-hosted runner credential workaround.** Default macOS Docker CLI defaults credential store to `osxkeychain`, which fails under launchd-spawned runner sessions ("keychain cannot be accessed because the current session does not allow user interaction"). Setting `credsStore: ""` in config.json or pointing `DOCKER_CONFIG` at an empty per-job dir is insufficient — Docker on darwin treats an unset/empty credsStore as "use platform default" = osxkeychain. Workaround in release.yml's "Isolate DOCKER_CONFIG" step: write `{"auths":{},"credsStore":"runnerstore"}` plus a 35-line bash credential helper (`docker-credential-runnerstore`) into `$RUNNER_TEMP/docker-config/bin/`, prepend that dir to `PATH`, and store/get persist via `$RUNNER_TEMP/docker-config/creds/<slug>.json`. `get` for unknown URLs returns `{"Username":"","Secret":""}` so anonymous pulls of public images (`moby/buildkit`) work; `docker/login-action`'s `store` payload survives for `bake --push` to consume on the same job. Inline self-test in the same step exercises round-trip + unknown-URL fallback before exporting env to subsequent steps. `$RUNNER_TEMP` cleaned by runner after job, so creds are ephemeral.

**Carryover / follow-ups:**

- PLE-370 → 371 → 372 → 373 README rewrite chain (M3 scope) and PLE-375 freshness badge are still open. `master` now reflects the shipped release but the README still has the pre-refresh structure — no decision matrix, no quickstart rewrite, no freshness badge.
- Branch-protection rules: signed-commits enforcement currently fires "Bypassed rule violations" on every push (master + feature branches). Working-as-intended for solo bypass-allowed config; flagged as a follow-up if the repo ever onboards another committer.
