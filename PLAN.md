...
| # | Name | Done criteria |
|---|---|---|
| **M1** | Modernize Foundation | `BUILD_ENV=test docker compose build` green on py3.12 / GHA for every kept target, amd64+arm64. Drill dropped, cstore_fdw replaced, structured logging, CWD paths fixed. |
...
3. **PLE-334** Bump Python 3.7→3.12 — touches every Dockerfile + host. Land before deps churn.
...
- **PLE-335** Delete CircleCI + README badges
...
- **PLE-332** AC overlaps PLE-334 (py3.12) + PLE-335 (delete CircleCI). Decision: keep PLE-332 scoped to ci.yml only; PLE-334/335 land separately.
